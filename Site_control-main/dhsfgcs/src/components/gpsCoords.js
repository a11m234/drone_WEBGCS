...
const DEFAULT_POSITION_STATE = {
  latitude_deg:  16.472429169395063,
  longitude_deg: 80.71801059545717,
  absolute_altitude_m: 0,
  relative_altitude_m: 0
 
};

function GpsCoords({ onGpsUpdate }) {
    ...
    const timer = setInterval(async () => {
      try {
        ...
        if (res.ok) {
          ...
          setGpsPos(newGpsPos);
          onGpsUpdate({ lat: newGpsPos.latitude_deg, lng: newGpsPos.longitude_deg });
        } else {
          console.error("Failed to fetch GPS coordinates");
        }
      } catch (error) {
